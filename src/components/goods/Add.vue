                         ...
                         <el-form-item :label="item.attr_name" v-for="(item) in manyTableData" :key="item.attr_id"> 
                          ...
                          <el-checkbox-group v-model="item.attr_vals">
                            <el-checkbox :label="cb" 
                              v-for="(cb,i) in item.attr_vals" :key="i" border>
                            </el-checkbox>
                          </el-checkbox-group>
                        </el-form-item>
                    </el-tab-pane>
...
export default {
    ...
    methods:{
     ...
     async   getCateList(){
       ...
       beforeTabLeave(activeName,oldActiveName){
        //   限制用户必须从一个tab栏去下一个tab栏，不能跳过
        if(activeName-oldActiveName!=1 && activeName > oldActiveName){
            this.$message.error("请一步一步按照步骤添加商品，不然会导致商品信息不完整")
            return false;
        }


        // 用户从第一个页面进入到其他页面时，需要验证
           if(oldActiveName==='0'){
               if(this.addForm.goods_cat.length!==3){
                   this.$message.error("请先选择商品三级分类，才能进去下一步");
                   return false;
               }
    ...
    //    监听图片上传成功的事件
      ...
      async add(){
        ...
        // 执行添加的业务逻辑，把goods_cat从数组转字符串，直接重新赋值会导致级联选择器报错
        // lodash   cloneDeep(obj) 深拷贝一个全新的对象
         const form=_.cloneDeep(this.addForm)
         form.goods_cat=form.goods_cat.join(',')

        //将manyTableData和onlyTableData 中的数据进行处理，添加到addForm.attrs数组中
        // 处理动态参数
